...
inspects an audit log it produces)?
  ...
  No  → continue
        ↑ This branch ALSO catches capacity operational docs — how to extend,
          ...
          to memory below, NOT to rules. (Example: capacity-internal rules
          that document hook-chain budgets, snapshot-pair mechanisms, or
          path-scoped load diagnostics are upstream-maintainer-only and belong
          in memory.)
...
The disposition criteria for future borderline audits (split vs move-full, canonical precedent pairs, the audit narratives that established the three-bucket model) live in the project memory companion entry for this rule.
