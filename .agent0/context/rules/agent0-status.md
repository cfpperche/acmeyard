...
- **`.agent0/tools/status.sh`** — the mid-session, untruncated sibling of the SessionStart brief. It reuses the brief's composition library (`.agent0/hooks/_brief-compose.sh`) to emit handoff (Current State / Active Work / Next Actions), due reminders, pending routines, memory decay, and actionable bootstrap advisories, then adds a git working-tree block and a derived `=== next ===` suggestion block. A **reconciliation layer** (spec 139) tops the output: a `⚠ RESUME WARNING` when the handoff claims clean/idle while the tree is dirty, plus a `probable active work: NNN-<slug>` line inferred from dirty `docs/specs/` paths — so the surface never silently contradicts itself. **Read-only; always exits 0.** Surfaced as the `/status` skill (Claude) / `$status` (Codex).
- **`.agent0/tools/doctor.sh`** — a harness health check. Per-check tri-state (`ok` / `advisory` / `broken`) over core files, project-core bootstrap/mirror drift, hook wiring, `core.hooksPath` activation, and required-vs-optional binaries, with a rollup line. Hook wiring is **contract-validated** (spec 139): jq confirms a `SessionStart` hook in `.claude/settings.json` / `.codex/hooks.json` actually binds an existing, executable `startup-brief.sh` — a present-but-unwired config is `broken`, not a substring pass. **Exits non-zero iff any check is `broken`**; optional-binary absence, pending project-core bootstrap, and project-core mirror drift are `advisory` and never fail the exit. Reports + proposes, never fixes (mirrors `vuln-audit`). Tool-only — no skill.
...
The shared bootstrap block is deliberately actionable-only: it appears when `.agent0/project-core.md.example` exists and `.agent0/project-core.md` is absent, and disappears once the source exists. If it keeps firing after bootstrap, treat that as stale context/a false positive rather than useful noise.

Configured consumers can still need review when Agent0 updates `.agent0/project-core.md.example`. In that case startup/status emit `=== project-core ===` and doctor emits an advisory until `.agent0/project-core.md` acknowledges the current `AGENT0:PROJECT-CORE-TEMPLATE` marker from the example. Mirror drift after the source exists belongs to `.agent0/tools/project-core-sync.sh --check` / `doctor.sh`, and repair is local: `.agent0/tools/project-core-sync.sh --apply`.
