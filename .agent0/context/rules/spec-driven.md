...
- **`debate.md`** — the *cross-model review* (opt-in). A structured transcript of a debate between the live Claude session and **another tool-calling CLI agent** (separate session — Codex CLI, Cursor, Aider, etc., each porting its own `/sdd debate` equivalent). Both agents read and write `debate.md` directly; the human alternates which agent is active and decides when the debate ends by explicitly asking for synthesis. No copy-paste, no broker process. The artifact IS the audit trail — no separate state file. Lives alongside the spec, git-tracked. Scaffolded and progressed by `/sdd debate` (see § *Workflow* step 1.5). Debate is always **decision-grade**, so it should run the spec-149 anti-confirmation-bias flow — blind commit/reveal Round 1 (de-anchor the reviewer), claim/evidence convergence gate (`assertion-only` ⇒ unresolved), preserved minority report — via the shared `meeting.sh` mechanics (see `.agent0/context/rules/meeting.md` § De-biased deliberation). The file is absent on specs where no debate was held; its absence carries no judgment.
