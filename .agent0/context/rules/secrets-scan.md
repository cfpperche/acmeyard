...
- **`.agent0/.runtime-state/agent-browser/state/*.json` are credential-class files.** The agent-browser auth flow (`adopt`, spec 152.2/153) saves session cookies + localStorage to `.agent0/.runtime-state/agent-browser/state/<host>.json`; leaking one is equivalent to leaking a password. The whole `.agent0/.runtime-state/` tree is gitignored (primary defense). If gitleaks detects a high-entropy string inside a state file that was accidentally staged, treat it as a real credential finding — see `.agent0/context/rules/browser-auth.md`.
