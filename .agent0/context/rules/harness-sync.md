...
If neither `--agent0-path` nor `AGENT0_HARNESS_PATH` is given, the tool refuses with exit code 2 and a usage hint naming both. (The explicit-path requirement exists because this script is in its own propagation manifest — an identical copy lives inside every consumer project, so the script's location cannot distinguish Agent0 source from consumer; maintainer-register detail lives in project memory.)
...
Motivating case: a public repo can consume Agent0 as local dev tooling without committing the harness or follow-on tracked drift.
...
`<consumer-path>/.agent0/harness-sync-baseline.json` records Agent0's managed-file sha-set plus Agent0-owned Claude settings hook identities as of the consumer project's last `--apply`. It is the third reference point that powers the 3-way reconciliation above and the ownership signal for safe `.claude/settings.json` hook removals. The baseline lives under `.agent0/` as the harness-home for runtime-neutral artifacts; the pre-migration `.claude/harness-sync-baseline.json` path is read as a fallback and removed on the next `--apply`. Shape:
...
```json
{
  ...
  "settings_hooks": [
    "[\"SessionStart\",\"\",[\"bash $CLAUDE_PROJECT_DIR/.agent0/hooks/startup-brief.sh\"]]"
  ]
}
```
...
- **Settings hook ownership set.** `settings_hooks` stores canonical JSON-string identities of Agent0-owned hook registrations from `.claude/settings.json`: `[event, matcher, ordered inner commands]`. The structured string avoids separator collisions while staying a simple array for Bash/JQ portability.
...
- **Written on every `--apply`** (not `--check`, not `--dry-run`), after all passes, atomically (`mktemp` + `mv`). Skipped when the resulting files-map and `settings_hooks` set are byte-identical to the existing baseline's — a no-op re-sync must leave the file untouched (idempotency), so `synced_at` is not churned.
...
3. **Agent0-owned top-level keys overwrite when Agent0 has them** — currently just `$schema`. When Agent0's settings declare it, the consumer project's value is replaced; when Agent0 doesn't declare it, consumer project's value (if any) is preserved untouched. (`statusLine` is not harness-owned; it lives in user-global `~/.claude/settings.json` per-developer.)
4. **`hooks` merge per-event** — for each top-level `hooks.<event>` array (`PreToolUse`, `PostToolUse`, `SessionStart`, `Stop`, `PreCompact`, …):
   - Remove a consumer hook entry only when its identity existed in the previous baseline's Agent0-owned `settings_hooks` set and is absent from current Agent0 settings. If the previous baseline lacks `settings_hooks`, prune nothing.
   - Concatenate remaining consumer entries with Agent0's current entries.
...
**Limitations:**
- Existing baselines without `settings_hooks` cannot prove hook ownership retroactively. The first apply after this metadata lands seeds ownership; future Agent0 hook removals/renames then prune safely.
- If `.claude/settings.json` is skipped (local-only) or Agent0's settings source is missing/unparseable, the sync preserves the previous `settings_hooks` baseline instead of advancing it to an empty set.
...
A fourth disposition, distinct from copy/merge/exclude: `COPY_CHECK_SEED`. A **seed** is a file that is git-tracked in Agent0 yet **regenerated locally** by the consumer's own tooling — so under the normal 3-way path it would read `!! customized` forever on every consumer that has run that tooling. The canonical case is the `/product` OD-engine (`sync-open-design.ts`), which rewrites three files with fresh timestamps/history on every `--apply`:
...
not registered as a recurring `UserPromptSubmit` hook — by design; the index + on-demand-read model is final
...
When a capacity's canonical home moves within Agent0 — relocations from `.claude/` to `.agent0/` follow the § Classification principle (maintainer-register detail lives in project memory) — propagation is **capacity-only**. The manifest carries the *capacity* (hooks, skills, rules, tools, and the empty `.gitkeep` scaffold of the new location), never the *content*. The posture is uniform across every relocation row, not specific to any one capacity:
...
**No upstream auto-migration of consumer content** — deliberate: auto-moving consumer content would cross the manifest's "never touch consumer/product files" floor. The cost is a documented one-time manual step per consumer project; the benefit is sync never mutates data it does not own.

## Project core (consumer-source mirror)

The two runtime entrypoints diverge in what each model sees: Claude Code reads `CLAUDE.md`, Codex (and every AGENTS.md-standard tool) reads `AGENTS.md` and never `CLAUDE.md`. Consumer project narrative authored in one entrypoint is invisible to the other runtime. The **project core** mechanism closes that gap for the small, always-needed core (project identity, voice, key conventions) by mirroring a single consumer-owned source into both entrypoints.
...
- **`settings.json` hook removals require prior ownership metadata.** A consumer with a v1/no-`settings_hooks` baseline is ambiguous, so sync preserves existing hook entries rather than guessing. After one apply writes v2 metadata, Agent0-owned removals/renames prune on later syncs while consumer-only hook identities remain.
...
- **First sync on a long-stale consumer project produces a large diff.** A consumer project that skipped several upstream releases will see ~30+ new files in one apply. Review the diff section-by-section: hooks first, rules second, tools third, then settings.json + CLAUDE.md. Commit in one go so the audit trail is clean.
...
- **An `--apply` that crashes mid-run (sync-harness self-overwrote) is recoverable.** A consumer project whose `sync-harness.sh` copy is very stale may crash on the `--apply` that updates it (bash was reading the old copy, which was overwritten mid-run). It is harmless: the new `sync-harness.sh` is already written, so re-running `--apply` completes cleanly. Every subsequent apply is single-run and crash-free.
...
- **`settings.json` references files OUTSIDE the manifest cause silent breakage in consumer projects.** Rule: whenever a tracked `.claude/settings.json` value references a file under `.claude/`, that file's directory MUST appear in `COPY_CHECK_RECURSIVE` or `COPY_CHECK_GLOBS`, otherwise consumer projects get a settings entry pointing at a non-existent path. When adding a new harness-owned top-level key to `settings.json` (beyond `hooks`/`$schema`), add it to the conditional `has(…)` block in `merge_settings_json` — otherwise the key won't propagate. The audit `git ls-files .claude/ | awk -F/ '{print $1"/"$2}' | sort -u` lists current subdirs; cross-check against the manifest arrays. (Dogfood history and the sub-bug case studies that established these rules live in project memory.)
