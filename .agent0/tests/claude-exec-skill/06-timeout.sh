...
TMPDIR="$(mktemp -d -t claude-exec-timeout-XXXXXX)"
...
FAKE_CLAUDE_PARTIAL_STDOUT='{"type":"system","subtype":"init"}' \
FAKE_CLAUDE_PARTIAL_STDERR="partial child stderr" \
FAKE_CLAUDE_SLEEP=5 \
...
assert_contains "$RUN_DIR/stdout.txt" '"type":"system"' "partial stdout is preserved"
...
set +e
CLAUDE_EXEC_STATE_DIR="$TMPDIR/bad-timeout-state" \
FAKE_CLAUDE_ARGS="$TMPDIR/bad-timeout-args.txt" \
FAKE_CLAUDE_STDIN="$TMPDIR/bad-timeout-stdin.txt" \
PATH="$TMPDIR/bin:$PATH" \
  bash "$AGENT0_ROOT/.agent0/skills/claude-exec/scripts/claude-exec.sh" \
    --permission-mode default \
    --timeout 0 \
...
assert_no_path "$TMPDIR/bad-timeout-args.txt" "invalid timeout blocks before invoking claude"
