#!/usr/bin/env bash
# .agent0/tests/memory-frontmatter-validate/run-all.sh
# Orchestrator for memory-frontmatter-validate.sh scenarios. Runs every
# NN-*.sh in order and prints a summary table. Exits 0 if all pass.
#
# Usage:
...
results=""
any_fail=0
tmpout="$(mktemp -t mfv-run-all-XXXXXX)"
...
# Summary table
# ---------------------------------------------------------------------------
printf '\n=== memory-frontmatter-validate scenario results ===\n'
printf '%s\n' "$results"
printf '====================================================\n'
