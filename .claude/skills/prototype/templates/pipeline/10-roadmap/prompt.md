...
**Output file:** `roadmap.md` in `docs/product/11-roadmap/`. Single-artifact — no `extra_files`.
...
- **PRD** — `docs/product/08-prd/prd.md` — § User Stories (with `US-NN` IDs) is the story surface to sequence; § Goals + § Success Metrics drive what "v1 is done" means; § Audit Response drives step-4 audit-driven scope; **PRD frontmatter `validation_mode`** picks the operating mode.
- **System design** — `docs/product/09-system-design/system-design.md` — § Services drives the build-dependency DAG (auth before features-needing-auth; data model before features-reading-data); § Stack drives the foundation-phase line items; § Integrations drives external-vendor sequencing (Stripe onboarding lag, Auth0 vs Supabase setup).
- **Cost estimate** — `docs/product/10-cost-estimate/cost-estimate.md` — § Build Cost RANGES (per phase) are the phasing-by-cost-pressure anchor; § Run Cost line items drive vendor-setup tasks in Foundation phase; § Recommendations are constraints (e.g. "Defer EU region" = NOT in v1 phases); § Sensitivity § Build-cost overrun is the buffer-calibration source.
- **Architecture JSON** — `docs/product/09-system-design/architecture.json` — quick scan of the components graph confirms the dependency ordering; nodes with no inbound edges go in Foundation.
