...
**Output file:** `<out>/docs/05-prd.md`. Single-artifact.
...
- **Upstream (informs this PRD):** `<out>/docs/01-concept-brief.md`, `<out>/docs/03-functional-spec.md`, `<out>/docs/04-validation-report.md`
- **Downstream (consumes this PRD):**
  - `<out>/docs/06-ost.md` — OST root = this PRD's NSM
  - `<out>/docs/07-sitemap.yaml` — every P0/P1 US-NN appears in ≥1 route's `covers_us`
  - `<out>/docs/08-system-design.md` — system-design references US-NN for scale assumptions
  - `<out>/docs/15-screen-atlas.md` — atlas coverage matrix lists every US-NN
...
- `.claude/skills/prototype/references/delegation-briefs.md` § Step 05 — full sub-agent brief
- `.claude/skills/prototype/references/pipeline-coverage.md` § Step 05 — size targets + lightening
