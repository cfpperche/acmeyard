...
**Goal:** produce `<out>/docs/12-gtm-launch.md` — a tight positioning canvas (April Dunford methodology) + 4-week launch plan sketch + pricing strategy. Stage-Gate stage 6 equivalent at standard tier per spec 045 Decision 7 (ported from spec 032). Closes Phase 2 (Specification) — gate fires after this step.
...
|---|---|---|---|
| `<out>/docs/12-gtm-launch.md` | positioning canvas + 4-week launch plan + pricing strategy | 4 KB | 7 KB |

## Inputs (read first)

- `<out>/docs/05-prd.md` § NSM + § User stories — target customer + value prop signal
- `<out>/docs/01-concept-brief.md` § Competitive positioning + § Monetization + § Growth loop — alternatives + tier hints
- `<out>/docs/10-roadmap.md` § Phase 1 — launch timing aligns with Phase 1 close (MVP ready milestone)
- `<out>/docs/09-legal-posture.md` § Regulated Aspects + § Privacy Posture — compliance signals affect launch claims (no "GDPR-compliant" claims without DPA in place; no "SOC 2 certified" without actual audit)
...
- `.claude/skills/prototype/references/delegation-briefs.md` § Step 12 — full sub-agent brief
- `.claude/skills/prototype/references/pipeline-coverage.md` § Step 12 — size targets + lightening
