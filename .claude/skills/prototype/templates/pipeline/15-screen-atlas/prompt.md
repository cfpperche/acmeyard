...
**Mode:** `synthesis` with `delegable: true`. Fully delegable — the sub-agent reads prior artifacts and produces the bundle with no live parent interview. The 5 structural signals (PRD US-NN list at `docs/05-prd.md` + **sitemap inventory at `docs/07-sitemap.yaml` (load-bearing — drives N screen-writer dispatches)** + tokens at `docs/14-tokens.css` + brand voice at `docs/13-brand-book.md` + audit findings at `docs/04-validation-report.md`) are mechanically extracted from prior artifacts; the synthesis is structural.
