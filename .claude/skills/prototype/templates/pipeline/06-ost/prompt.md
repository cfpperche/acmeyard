...
**Goal:** produce `<out>/docs/06-ost.md` — a structured Opportunity Solution Tree (Teresa Torres, *Continuous Discovery Habits*) connecting the PRD's North Star Metric to user-problem opportunities to candidate solutions. Sibling artifact to PRD (not embedded) per spec 045 Decision 12 (ported from spec 032).
...
|---|---|---|---|
| `<out>/docs/06-ost.md` | structured opportunity tree linking NSM → opportunities → solutions | 3 KB | 6 KB |

## Inputs (read first)

- `<out>/docs/05-prd.md` § Success metrics (NSM slot) — the desired-outcome ROOT of the tree
- `<out>/docs/05-prd.md` § User stories — context for what's already in scope
- `<out>/docs/03-functional-spec.md` § Problem-Validation Interviews — raw problem signal (3-5 summaries per Decision 6)
- `<out>/docs/01-concept-brief.md` § Mechanics + § Hook — persona context for inference fallback when interviews are thin
...
- `.claude/skills/prototype/references/delegation-briefs.md` § Step 06 — full brief
- `.claude/skills/prototype/references/pipeline-coverage.md` § Step 06 — size targets + lightening
