...
When the OD vendor is unavailable — `product_design_systems_index` → `code: "od-vendor-missing"` (broken install) or `code: "od-disabled"` (deliberately switched off via `PRODUCT_PIPELINE_OD=off`) — the manual-escape path still cites named systems, but without vendored paths the `design-systems/` Layer 1 check cannot pass. In that case the agent includes the substring `design-systems/` in a comment documenting the escape (`<!-- od-disabled (or od-vendor-missing): directions grounded in training-data knowledge of design-systems/; see pipeline.md § Manual escape -->`) so the escape is visible rather than silent.
