...
Every `Agent` tool call dispatched by `/prototype` v3 MUST use the 5-field handoff per `.claude/rules/delegation.md` (TASK / CONTEXT / CONSTRAINTS / DELIVERABLE / DONE_WHEN). The delegation-gate hook returns exit 2 otherwise.
...
- Write file DIRECTLY to {{out}}/docs/01-concept-brief.md. Do NOT create extra files.

DELIVERABLE: {{out}}/docs/01-concept-brief.md
...
- Write file DIRECTLY to {{out}}/docs/02-direction-a.html. The 3-5 killer-flow mood screens are produced by separate per-stack screen-writer dispatches (sub-agent b — see § Per-stack screen-writer below).

DELIVERABLE: {{out}}/docs/02-direction-a.html (+ killer-flow HTML mood screens at {{out}}/docs/02-screens/NN-<name>.html produced by sub-agent b in parallel)
...
CONTEXT: Read concept-brief.md at {{out}}/docs/01-concept-brief.md for product scope. Read direction-a.html at {{out}}/docs/02-direction-a.html + screens at {{out}}/docs/02-screens/ for surface inventory. Read .claude/skills/prototype/templates/pipeline/03-spec/prompt.md for canonical structure (standard tier combines spec + architecture to 8-12 KB single file). Read .claude/skills/prototype/templates/pipeline/03-spec/schema.md.
...
- ≥ 3 Gherkin scenarios.
- Write file DIRECTLY to {{out}}/docs/03-functional-spec.md.

DELIVERABLE: {{out}}/docs/03-functional-spec.md
...
CONTEXT: Read direction-a.html at {{out}}/docs/02-direction-a.html + screens at {{out}}/docs/02-screens/ for rendered surfaces (PROJECTED-mode audit at standard tier). Read functional-spec.md at {{out}}/docs/03-functional-spec.md for declared behavior. Read .claude/skills/prototype/templates/pipeline/04-ux-testing/prompt.md + schema.md.
...
- ≥ 3 findings minimum.
- 5-8 KB hard ceiling.
- Write file DIRECTLY to {{out}}/docs/04-validation-report.md.

DELIVERABLE: {{out}}/docs/04-validation-report.md (with YAML frontmatter)
...
CONTEXT: Read concept-brief.md + functional-spec.md + validation-report.md frontmatter + direction-a.html + screens at {{out}}/docs/ for product scope. Read .claude/skills/prototype/templates/pipeline/05-prd/prompt.md + schema.md for the Lenny hybrid shape.
...
- Write file DIRECTLY to {{out}}/docs/05-prd.md.

DELIVERABLE: {{out}}/docs/05-prd.md
...
CONTEXT: Read prd.md at {{out}}/docs/05-prd.md for NSM (desired outcome) + user stories + anti-goals. Read functional-spec.md at {{out}}/docs/03-functional-spec.md § Problem-Validation Interviews for raw problem signal. Read concept-brief.md at {{out}}/docs/01-concept-brief.md for persona context. Read .claude/skills/prototype/templates/pipeline/06-ost/prompt.md for canonical OST shape. Reference: Teresa Torres, Continuous Discovery Habits (Product Talk Academy).
...
- 3-6 KB hard ceiling.
- Write file DIRECTLY to {{out}}/docs/06-ost.md.

DELIVERABLE: {{out}}/docs/06-ost.md
...
CONTEXT: Read prd.md at {{out}}/docs/05-prd.md for US-NN inventory. Read functional-spec.md at {{out}}/docs/03-functional-spec.md § Pages & Surfaces for surface inventory. Read concept-brief.md at {{out}}/docs/01-concept-brief.md for product class (B2C / B2B / internal-tool / etc — drives which required_categories apply). Read .claude/skills/prototype/references/sitemap-schema.md for the binding schema. Read .claude/skills/prototype/templates/pipeline/07-sitemap-ia/prompt.md + schema.md for canonical shape.
...
- 2-5 KB hard ceiling.
- Write file DIRECTLY to {{out}}/docs/07-sitemap.yaml.

DELIVERABLE: {{out}}/docs/07-sitemap.yaml
...
CONTEXT: Read prd.md at {{out}}/docs/05-prd.md (scope drives scale assumption) + sitemap.yaml at {{out}}/docs/07-sitemap.yaml (route inventory drives integration list + auth requirements) + functional-spec.md at {{out}}/docs/03-functional-spec.md (preliminary architecture) + concept-brief.md at {{out}}/docs/01-concept-brief.md (product class + audience). Read .claude/skills/prototype/templates/pipeline/08-system-design/prompt.md + schema.md.
...
DELIVERABLE: 3 files: {{out}}/docs/08-system-design.md + {{out}}/docs/08-security.md + {{out}}/docs/08-data-flow.json
...
CONTEXT: Read prd.md at {{out}}/docs/05-prd.md (audience drives jurisdiction exposure) + system-design.md at {{out}}/docs/08-system-design.md (Integrations name every sub-processor) + **data-flow.json at {{out}}/docs/08-data-flow.json (parses flows[]; if any flow has data_categories ⊃ {pii, health, minors, financial}, DPIA section is MANDATORY)** + concept-brief.md at {{out}}/docs/01-concept-brief.md (audience). Read .claude/skills/prototype/templates/pipeline/09-legal/prompt.md + schema.md.
...
- Write file DIRECTLY to {{out}}/docs/09-legal-posture.md.

DELIVERABLE: {{out}}/docs/09-legal-posture.md
...
CONTEXT: Read prd.md at {{out}}/docs/05-prd.md (user stories + priorities) + system-design.md at {{out}}/docs/08-system-design.md (dependencies + integrations driving build sequence) + concept-brief.md at {{out}}/docs/01-concept-brief.md (product class) + validation-report.md at {{out}}/docs/04-validation-report.md (validation_mode drives canonical-vs-bridge mode). Read .claude/skills/prototype/templates/pipeline/10-roadmap/prompt.md + schema.md.
...
- 5-8 KB hard ceiling.
- Write file DIRECTLY to {{out}}/docs/10-roadmap.md.

DELIVERABLE: {{out}}/docs/10-roadmap.md
...
CONTEXT: Read **roadmap.md at {{out}}/docs/10-roadmap.md (phase boundaries drive cost calculation — load-bearing for per-phase breakdown)** + system-design.md at {{out}}/docs/08-system-design.md (stack + integrations drive line items) + legal-posture.md at {{out}}/docs/09-legal-posture.md (DPIA + counsel review budget) + prd.md at {{out}}/docs/05-prd.md (success metric drives scale assumption). Read .claude/skills/prototype/templates/pipeline/11-cost-estimate/prompt.md + schema.md.
...
- Write file DIRECTLY to {{out}}/docs/11-cost-estimate.md.

DELIVERABLE: {{out}}/docs/11-cost-estimate.md
...
CONTEXT: Read prd.md at {{out}}/docs/05-prd.md (NSM + audience for positioning) + concept-brief.md at {{out}}/docs/01-concept-brief.md (competitive positioning + monetization tier hints) + roadmap.md at {{out}}/docs/10-roadmap.md (launch timing aligns with roadmap Phase 1 close) + legal-posture.md at {{out}}/docs/09-legal-posture.md (compliance signals affect launch claims). Read .claude/skills/prototype/templates/pipeline/12-gtm-launch/prompt.md + schema.md. Reference: April Dunford, Obviously Awesome.
...
- Write file DIRECTLY to {{out}}/docs/12-gtm-launch.md.

DELIVERABLE: {{out}}/docs/12-gtm-launch.md
...
CONTEXT: Read prd.md at {{out}}/docs/05-prd.md (finalized scope + NSM + persona) + gtm-launch.md at {{out}}/docs/12-gtm-launch.md (positioning canvas already locked — brand voice should reinforce, not contradict) + concept-brief.md at {{out}}/docs/01-concept-brief.md (audience + product class) + direction-a.html at {{out}}/docs/02-direction-a.html (visual lineage). Read .claude/skills/prototype/templates/pipeline/13-brand/prompt.md for canonical 7-section structure (we target 2-3 section snapshot at standard tier).
...
- 4-8 KB hard ceiling.
- Write file DIRECTLY to {{out}}/docs/13-brand-book.md.

DELIVERABLE: {{out}}/docs/13-brand-book.md
...
```
TASK: Produce 14-tokens.css + 14-components.md + 14-design-system.md applying the brand-book to concrete semantic design tokens for "{{idea}}". Catalog-path PREFERRED (cite 1-2 OD vendors).

CONTEXT: Read brand-book.md at {{out}}/docs/13-brand-book.md for posture + voice. Read sitemap.yaml at {{out}}/docs/07-sitemap.yaml for component scope (what surfaces need styling). Read concept-brief.md at {{out}}/docs/01-concept-brief.md for product class. Read .claude/skills/prototype/references/od-catalog-index.json for the 72-vendor catalog — pick 1-2 vendors whose mood + category match the brand-book; their DESIGN.md path (vendor_path field) is the lineage citation source. Read validation-report.md at {{out}}/docs/04-validation-report.md frontmatter `findings[]` and filter `fix_skill_hint: "design-system"` — these are token tunes to apply. Read .claude/skills/prototype/templates/pipeline/14-design-system/prompt.md + schema.md.
...
- **tokens.css written to {{out}}/docs/14-tokens.css** (NOT root — root is reserved for runtime per spec 036 finding #7 iter-2). The skeleton's `app/globals.css` imports it relative as `@import "../docs/14-tokens.css"`.
...
- design-system.md: overview + tokens narrative + audit-response section (which step-04 findings applied as token tunes) + catalog lineage citations. 8+ KB. Required H2: "Audit Response".
- Write 3 files DIRECTLY to {{out}}/docs/: 14-tokens.css + 14-components.md + 14-design-system.md.

DELIVERABLE: 3 files at {{out}}/docs/: 14-tokens.css + 14-components.md + 14-design-system.md

DONE_WHEN: 14-tokens.css ≥ 1.5 KB valid CSS with :root block + light-mode @media override; 14-components.md ≥ 3 KB; 14-design-system.md ≥ 8 KB + contains "Audit Response" section header + cites OD vendor name + vendor_path.
```
...
CONTEXT: Read ALL prior artifacts at {{out}}/docs/ (sequential 01..14 prefix):
- 01-concept-brief.md, 03-functional-spec.md, 04-validation-report.md (Phase 1)
- 05-prd.md (US-NN inventory — load-bearing for PRD coverage), 06-ost.md, 07-sitemap.yaml (route inventory — load-bearing for screen coverage), 08-system-design.md, 09-legal-posture.md (legal-mandatory surfaces — consent dialog if DPIA fires), 10-roadmap.md, 11-cost-estimate.md, 12-gtm-launch.md (Phase 2)
- 13-brand-book.md, 14-tokens.css, 14-components.md, 14-design-system.md (Phase 3)
Read .claude/skills/prototype/templates/pipeline/15-screen-atlas/prompt.md + schema.md for atlas shape.
...
- This atlas IS the brand+tokens-applied hi-fi pass (Step 7 prototype-v2 from v2 was deleted per spec 045 Decision 8). The screens MUST consume var(--*) tokens from `docs/14-tokens.css`, NOT raw hex.
- Write file DIRECTLY to {{out}}/docs/15-screen-atlas.md.

DELIVERABLE: {{out}}/docs/15-screen-atlas.md
...
CONTEXT:
- Pipeline step context: {{step_label}} (02 = lo-fi pre-brand mood HTML at {{out}}/docs/02-screens/, 15 = hi-fi brand-tuned page.tsx in {{out}}/app/).
- Sitemap entry source: {{out}}/docs/07-sitemap.yaml (route fields: path / category / states / covers_us / components)
- Tokens (Step 15 only): {{out}}/docs/14-tokens.css (consumed via the `app/globals.css` `@import "../docs/14-tokens.css"` line — use `var(--color-*)` inline / Tailwind utility-classes that resolve to the token names)
- Voice (Step 15 only): {{out}}/docs/13-brand-book.md (match ON-brand voice for copy)
- Components reference (Step 15 only): {{out}}/docs/14-components.md
- Stack defaults: .claude/skills/prototype/references/stack-defaults.md § Next.js
- Target file:
  - Step 02: {{out}}/docs/02-screens/{{NN}}-{{name}}.html (self-contained HTML, inline styles, mood-only)
