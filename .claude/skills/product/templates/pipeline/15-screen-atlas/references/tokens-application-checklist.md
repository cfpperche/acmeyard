...
## The substitution contract (step 15 inherits from step 14)

Step 14's `docs/design-system/tokens.css` is the canonical value layer. Every color / type / spacing / radius / shadow value in step-15 screens resolves to a `var(--token)` defined there. Per spec 045 Decision 8 + 14, the deleted v2 Step 7 intermediate prototype is absorbed into Step 15 — there is NO separate intermediate brand-applied pass. Step 15 picks ONE substitution path (Path A direct-rename OR Path B alias-in-`:root`) consistently across all per-route page.tsx files.

**Inheritance discipline:** all step-15 `app/<route>/page.tsx` files apply the same path (A or B). Mixed path is a discipline failure — engineering opening one page.tsx with Path A and a sibling page with Path B can't grep consistently across the atlas.

**To verify path consistency:** open any `app/<route>/page.tsx`. If `style={{ color: 'var(--color-foreground)' }}` directly resolves to step-14 semantic names with no intermediate aliasing, Path A. If component-level CSS variables alias the step-14 tokens (`--button-bg: var(--color-primary)`), Path B. Pick one and propagate.
...
For every screen `docs/screens/<NN>-<name>.html`, the agent runs this checklist before submitting:

### 1. `:root` block is verbatim from step 6 tokens.css

- [ ] Open `docs/design-system/tokens.css`. Copy the `:root { ... }` content verbatim into the screen's `<style>` block.
...
- [ ] Open `docs/brand-book.md` § Voice samples (ON-brand and OFF-brand pairs).
...
- [ ] Open `docs/design-system/components.md`. For each component on the screen (button, input, modal, list, card, toast, etc.), confirm:
