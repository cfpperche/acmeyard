...
- **Visual verification.** Each child verifies its screens against the atlas + hi-fi mood with the `agent-browser` primitive (`agent-browser.sh audit … --structure optional` over `file://`; no MCP, spec 153). Screenshot at 375 px + 1280 px; check horizontal overflow.
